...
# "do week X". Running it by hand is harmless and idempotent.
set -euo pipefail
...
active_week() {
  [[ -f "$dropper/.active-week" ]] || die "no dropper/.active-week. Run ./scripts/set-week.sh WeekXX"
  local w
  w="$(tr -d '[:space:]' < "$dropper/.active-week")"
  [[ "$w" =~ ^Week[0-9][0-9]$ ]] || die "dropper/.active-week is malformed ('$w'). Run ./scripts/set-week.sh WeekXX"
  [[ -d "$root/$w" ]] || die "$w does not exist. Run ./scripts/set-week.sh $w"
  printf '%s' "$w"
}
...
cmd_scan() {
  local week; week="$(active_week)"
  local mdir="$root/$week/materials"
  ...
  for f in "$dropper"/*; do
    ...
    base="$(basename "$f")"
    case "$base" in
      .active-week|_intake-proposal.tsv|.gitkeep|.DS_Store) continue ;;
    esac
    found=$((found + 1))
    sha="$(sha_of "$f")"
  ...
  if [[ "$found" -eq 0 ]]; then
    echo "NOTHING_TO_INTAKE week=$week"
    return 0
  fi
  echo "PROPOSAL week=$week file=$proposal"
  cat "$proposal"
}
...
cmd_apply() {
  ...
  write_intake_md "$week" "$manifest"
  echo "APPLIED week=$week moved=$moved manifest=${manifest#$root/}"
}

# Human-readable provenance log, regenerated from the manifest each apply.
write_intake_md() {
  local week="$1" manifest="$2" out="$root/$week/materials/INTAKE.md"
  {
    echo "# $week — intake log"
    echo
